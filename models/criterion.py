...
class SetCriterion(nn.Module):
    ...
    def loss_class(self, pred_logits, annotations, indices, num_boxes):
        ...
        target_classes_o = torch.cat([t["labels"][J] for t, (_, J) in zip(annotations, idx)])
        target_classes = torch.full(
            src_logits.shape[:2], self.num_classes, dtype=torch.int64, device=src_logits.device
        )
        target_classes[idx] = target_classes_o
        loss_ce = self.sigmoid_focal_loss(src_logits, target_classes, num_boxes)
        return loss_ce  

    def loss_boxes(self, pred_boxes, annotations, indices, num_boxes):
        idx = self._get_src_permutation_idx(indices)
        src_boxes = pred_boxes[idx]
        target_boxes = torch.cat([t["boxes"][i] for t, (_, i) in zip(annotations, idx)], dim=0)
    ...
    def loss_giou(self, pred_boxes, annotations, indices, num_boxes):
        ...
        src_boxes = pred_boxes[idx]
        target_boxes = torch.cat([t["boxes"][i] for t, (_, i) in zip(annotations, idx)], dim=0)
        loss_giou = self.generalized_box_iou_loss(src_boxes, target_boxes)
        loss_giou = loss_giou.sum() / num_boxes
        return loss_giou

    def loss_domains(self, out, domain_label):
        loss_domain = F.binary_cross_entropy_with_logits(out, domain_label)
        return loss_domain
    ...
    def forward(self, out, annotations=None, domain_label=None, enable_mae=False):
        ...
        loss_class = self.loss_class(out["pred_logits"], annotations, indices, num_boxes)
        loss_boxes = self.loss_boxes(out["pred_boxes"], annotations, indices, num_boxes)
        loss_giou = self.loss_giou(out["pred_boxes"], annotations, indices, num_boxes)
        loss_domains = self.loss_domains(out["pred_domains"], domain_label)
        loss = self.coef_class * loss_class + self.coef_boxes * loss_boxes + self.coef_giou * loss_giou + self.coef_domain * loss_domains
        loss_dict = {
